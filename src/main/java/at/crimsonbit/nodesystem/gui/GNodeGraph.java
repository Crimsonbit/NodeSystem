	...
	private void onSave() {
		...
		if (f != null)
			try {
				...
				Toast.makeToast((Stage) getScene().getWindow(), "NodeSystem saved successfully!", ToastTime.TIME_SHORT, ToastPosition.BOTTOM);
			} catch (IOException e) {
				Toast.makeToast((Stage) getScene().getWindow(), "Error while saving!", ToastTime.TIME_SHORT, ToastPosition.BOTTOM);
			}
		else
			Toast.makeToast((Stage) getScene().getWindow(), "Error file is null!", ToastTime.TIME_SHORT, ToastPosition.BOTTOM);
	}
